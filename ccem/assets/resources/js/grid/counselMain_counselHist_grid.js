$(function(){
	...
	counselMain_studyProgressList_grid.on('dblclick', (ev) => {
		if (ev["targetType"] == "cell") {
        	var rowData = 	counselMain_studyProgressList_grid.getRow(ev.rowKey);
        	var settings = {
	...
	// ipcc 리스트 응답 결과 수신
	counselMain_studyProgressList_grid.on('response', ev => {
	...
	counselMain_studyList_grid.on('dblclick', (ev) => {
		if (ev["targetType"] == "cell") {
        	var rowData = 	counselMain_studyList_grid.getRow(ev.rowKey);
        	var settings = {
					url: API_SERVER + "/srch/IPCCInfo",
					method: "GET",
					dataType : "json",
	...
	// ipcc 리스트 응답 결과 수신
	counselMain_studyList_grid.on('response', ev => {
		const {response} = ev.xhr;
		const responseObj = JSON.parse(response);
		console.log('data : ', responseObj.result);
		console.log('data : ', responseObj.data);
