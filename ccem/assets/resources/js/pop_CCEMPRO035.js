...
const createGrids = () => {
	...
	grid1 = new Grid({
		...
		columns: [
			...
			{ header: '녹취ID',                         name: "RECORD_ID",             width: 150,    align: "center",    sortable: true,    ellipsis: true,    hidden: false, 	 formatter: function(data){ var text = ``; if ( data.value==null ) text =``; else if ( data.value=="MOREDATA" ) text += `<button style="padding: 0px;" class="btn btn-sm navBtn" type="button"><span>선택청취</span></button>`; else text += `<button style="padding: 0px;" class="btn btn-sm navBtn" type="button"><span>청취</span></button>`; return text; }																  },
